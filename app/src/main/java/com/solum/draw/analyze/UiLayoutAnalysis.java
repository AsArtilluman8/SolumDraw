...
public final class UiLayoutAnalysis {
    ...
    public Bitmap makeUiMap(Bitmap source) {
        Bitmap bmp = scale(source, 360).copy(Bitmap.Config.ARGB_8888, true);
        Canvas c = new Canvas(bmp);
        Paint wash = new Paint(Paint.ANTI_ALIAS_FLAG);
        wash.setColor(0x99000000);
        ...
        for (Region r : regions) {
            ...
            c.drawRect(rr, p);
            c.drawText(r.role + " " + Math.round(r.score * 100f) + "%", rr.left + 5, Math.max(18, rr.top + 18), t);
        }
        return bmp;
    }
