...
public final class DrawingIntentAnalysis {
    ...
    public static DrawingIntentAnalysis build(ImageAnalysis a, AnalysisLayers layers, ComponentRoleMap roles, UiLayoutAnalysis ui) {
        float uiEvidence = clamp01(ui.uiScore * 0.70f + (a.genre.equals("game_engine_ui") || a.genre.equals("ui_screenshot") ? 0.28f : 0f));
        float textEvidence = clamp01((a.realTextRatio * 0.85f) + Math.min(0.40f, a.textLineCount * 0.045f) + (ui.textScore * 0.22f));
        ...
        float characterEvidence = clamp01((a.genre.equals("anime_cartoon_flat") ? 0.34f : 0f) + a.centralObjectRatio * 0.26f + a.saturation * 0.32f + a.glyphRatio * 0.10f + Math.max(0f, 0.24f - ui.uiScore));
        float sceneEvidence = clamp01((a.genre.equals("dark_cinematic") || a.genre.equals("digital_art_wallpaper") || a.genre.equals("general_photo_or_illustration") ? 0.20f : 0f) + a.detailDensity * 0.24f + a.edgeDensity * 0.14f + ui.sceneScore * 0.32f + Math.max(0f, 0.30f - ui.panelScore) * 0.40f);

        if (uiEvidence > 0.58f && characterEvidence > 0.48f) {
            risksAdd(null); // keep method referenced by compiler? no-op via overload below.
        }
        ...
        List<String> layersOut = semanticLayersFor(intent, a, ui);
        List<String> order = drawOrderFor(intent);
        List<String> hints = hintsFor(intent, a, ui);
        ...
        int actions = estimateActions(intent, a, ui);
    ...
    private static List<String> semanticLayersFor(String intent, ImageAnalysis a, UiLayoutAnalysis ui) {
        ArrayList<String> s = new ArrayList<>();
        s.add("BACKGROUND");
        if (intent.equals("ui_layout_first") || intent.equals("ui_candidate_review") || intent.equals("text_layout_first")) {
            s.add("SCENE_OR_APP_BACKGROUND"); s.add("LARGE_UI_PANELS"); s.add("CARDS_OR_BUTTONS"); s.add("ICONS"); s.add("TEXT_LABELS"); s.add("FOCUS_CURSOR_OR_GIZMO"); s.add("IGNORE_DECORATIVE_NOISE");
        } else if (intent.equals("character_silhouette_first")) {
            s.add("MAIN_CHARACTER_SILHOUETTE"); s.add("SKIN_OR_FACE_REGION"); s.add("HAIR_OR_HEAD_MASS"); s.add("CLOTHES_COLOR_MASS"); s.add("OUTER_CONTOUR"); s.add("FACE_DETAILS"); s.add("HIGHLIGHTS"); s.add("IGNORE_TEXTURE_NOISE");
        } else if (intent.equals("portrait_mass_first")) {
            s.add("HEAD_SHAPE"); s.add("SKIN_LIGHT_SHADOW_MASS"); s.add("HAIR_MASS"); s.add("EYES_NOSE_MOUTH"); s.add("SOFT_SHADOWS"); s.add("EDGE_ACCENTS"); s.add("IGNORE_SENSOR_NOISE");
        } else if (intent.equals("logo_shape_first")) {
            s.add("BACKGROUND_SHAPE"); s.add("GLOW_OR_ACCENT_MASS"); s.add("MAIN_SYMBOL"); s.add("INNER_CUTS"); s.add("CRISP_EDGES"); s.add("HIGHLIGHTS"); s.add("IGNORE_COMPRESSION_NOISE");
        } else if (intent.equals("contour_lineart_first")) {
            s.add("PRIMARY_CONTOURS"); s.add("STRUCTURE_LINES"); s.add("SECONDARY_DETAILS"); s.add("CROSS_LINES"); s.add("CLEANUP"); s.add("IGNORE_PAPER_NOISE");
        } else if (intent.equals("flat_shape_first")) {
            s.add("LARGE_FLAT_COLOR_REGIONS"); s.add("MEDIUM_SHAPES"); s.add("CLEAN_BOUNDARIES"); s.add("ACCENT_SHAPES"); s.add("SMALL_DETAILS");
        } else if (intent.equals("scene_mass_first")) {
            s.add("SKY_OR_FAR_BACKGROUND"); s.add("GROUND_OR_ARCHITECTURE_MASS"); s.add("PERSPECTIVE_LINES"); s.add("MAIN_OBJECTS"); s.add("LIGHT_AND_SHADOW"); s.add("DETAIL_CLUSTERS"); s.add("IGNORE_TINY_TEXTURE_NOISE");
        } else {
            s.add("LARGE_COLOR_MASS"); s.add("MAIN_SUBJECT"); s.add("OUTER_CONTOUR"); s.add("INNER_DETAIL"); s.add("HIGHLIGHT"); s.add("IGNORE_NOISE");
        }
        return s;
    }
    ...
    private static List<String> drawOrderFor(String intent) {
        ...
        if (intent.equals("ui_layout_first") || intent.equals("ui_candidate_review") || intent.equals("text_layout_first")) {
            add(o, "background/app backdrop", "main viewport or app content", "large panels", "cards/buttons", "icons", "text labels", "cursor/gizmo", "polish");
        } else if (intent.equals("character_silhouette_first")) {
            add(o, "background", "body silhouette", "large color masses", "outer contour", "face/hair priority details", "clothes details", "highlights", "polish");
        } else if (intent.equals("portrait_mass_first")) {
            add(o, "background", "head/skin masses", "hair mass", "facial structure", "eyes/nose/mouth", "soft shadows", "edge accents", "polish");
        } else if (intent.equals("logo_shape_first")) {
            add(o, "background shape", "glow/accent mass", "main symbol", "inner cuts", "crisp edges", "highlights", "texture/noise only if useful");
        } else if (intent.equals("contour_lineart_first")) {
            add(o, "main contours", "big structure lines", "secondary contours", "small details", "cleanup", "polish");
        } else if (intent.equals("flat_shape_first")) {
            add(o, "largest flat shapes", "medium color regions", "clean boundaries", "accent shapes", "small details", "polish");
        } else if (intent.equals("scene_mass_first")) {
            add(o, "far background", "large scene masses", "perspective structure", "main objects", "light/shadow blocks", "detail clusters", "polish");
        } else {
            add(o, "background", "large masses", "main subject", "outer contour", "inner detail", "highlight", "polish");
        }
        return o;
    }

    private static List<String> hintsFor(String intent, ImageAnalysis a, UiLayoutAnalysis ui) {
    ...
    private static int estimateActions(String intent, ImageAnalysis a, UiLayoutAnalysis ui) {
        int base = 70;
    ...
    private static void kv(StringBuilder b, String k, String v, boolean comma) { b.append("  \"").append(k).append("\": \"").append(esc(v)).append("\""); if (comma) b.append(','); b.append('\n'); }
    ...
    private static void array(StringBuilder b, String k, List<String> list, boolean comma) { b.append("  \"").append(k).append("\": ["); for (int i = 0; i < list.size(); i++) { if (i > 0) b.append(','); b.append("\"").append(esc(list.get(i))).append("\""); } b.append(']'); if (comma) b.append(','); b.append('\n'); }
    ...
    private static void risksAdd(Object ignored) {}
}
